contract("SGRWalletsTradingLimiterUnitTest", function(accounts) {
    ...
    const updateWalletAllowedIdentifier = "ISGRTokenManager";
    ...
    var testingValues = [
     {
        "side" : "buy",
        "testedLimiter": "SGRBuyWalletsTradingLimiter",
        "dataSourceIdentifier": "BuyWalletsTradingDataSource"
     },
     {
        "side" : "sell",
        "testedLimiter": "SGRSellWalletsTradingLimiter",
     ...
     for (let j = 0; j < testingValues.length; j++) {
        ...
        describe("function getWalletsTradingDataSourceIdentifier:", function() {
           it("should return the right data source identifier", async function() {
              if (testedLimiter == "SGRBuyWalletsTradingLimiter")
                assert.equal("BuyWalletsTradingDataSource", web3.toUtf8(await sgrWalletsTradingLimiter.walletsTradingDataSourceIdentifier.call()));
              else
                assert.equal("SellWalletsTradingDataSource", web3.toUtf8(await sgrWalletsTradingLimiter.walletsTradingDataSourceIdentifier.call()));
           });
        });
        ...
        describe("function getOverrideTradeLimitAndClass:", function() {
           it("should return the authorizationDataSource getSellTradeLimitAndClass value", async function() {
              ...
              var [actualLimit, _] = await sgrWalletsTradingLimiter.getOverrideTradeLimitAndClass(wallet);
        ...
        describe("function getTradeLimit:", function() {
           it("should return the trading classes value", async function() {
              ...
              var actualLimit = await sgrWalletsTradingLimiter.getTradeLimit(tradeClassId);
        ...
        describe("function updateWallet:", function() {
           it("function updateWallet should abort with an error if called by a non-user", async function() {
               await catchRevert(sgrWalletsTradingLimiter.updateWallet(wallet, VALUE, {from: nonOwner}));
           });
           ...
           it("function updateWallet should complete successfully when the buy-limit is zero", async function() {
               ...
               await sgrWalletsTradingLimiter.updateWallet(wallet, VALUE);
           });
        ...
        describe("function getUpdateWalletPermittedContractLocatorIdentifier:", function() {
           it("should return updateWalletAllowedIdentifier", async function() {
            ...
            assert.equal(updateWalletAllowedIdentifier, web3.toUtf8(await sgrWalletsTradingLimiter.getUpdateWalletPermittedContractLocatorIdentifier()));
           });
        });

        describe("function getLimiterValue:", function() {
           it("should convert the input SGR amount to limiter amount", async function() {
               assert.equal(await sgrWalletsTradingLimiter.getLimiterValue(VALUE) , VALUE);
               await walletsTradingLimiterValueConverter.setRatio(2);
               assert.equal(await sgrWalletsTradingLimiter.getLimiterValue(VALUE) ,  2*VALUE);
           });
        });
 });
}
 ...
 async function rebuild(testedLimiter, dataSourceIdentifier) {
     ...
     sgrWalletsTradingLimiter              = await artifacts.require(testedLimiter                   ).new(contractAddressLocatorProxy.address);
