...
FIXED_ONE = 0x20000000000000000000000000000000;
A_B_SCALE = 10000000000000000000000000000000000;
...
def getNewN(_newR, _minR, _minN, _alpha, _beta):
    temp = pow(_newR * FIXED_ONE, _minR, _alpha, A_B_SCALE);
    return _alpha * temp // (_alpha * FIXED_ONE // _minN + _beta * (temp - FIXED_ONE));
...
def getNewR(_newN, _minN, _minR, _alpha, _beta):
    ...
    return pow(temp1 * FIXED_ONE // temp2 * _newN, _minN, A_B_SCALE, _alpha) * _minR // FIXED_ONE;

'''
    Return (a / b / FIXED_ONE) ^ (c / d) * FIXED_ONE
'''
def pow(_a, _b, _c, _d):
    return exp(log(_a // _b) * _c // _d);
...
def exp(_x):
    ...
    if ((_x & 0x04000000000000000000000000000000) != 0): res = res * 0x70f5a893b608861e1f58934f97aea5816 // 0x63afbe7ab2082ba1a0ae5e4eb1b479e04; # multiply by e^2^(-3)
    if ((_x & 0x08000000000000000000000000000000) != 0): res = res * 0x63afbe7ab2082ba1a0ae5e4eb1b479e11 // 0x4da2cbf1be5827f9eb3ad1aa9866ebb76; # multiply by e^2^(-2)
    if ((_x & 0x10000000000000000000000000000000) != 0): res = res * 0x4da2cbf1be5827f9eb3ad1aa9866ebb8b // 0x2f16ac6c59de6f8d5d6f63c1482a7c89d; # multiply by e^2^(-1)
    if ((_x & 0x20000000000000000000000000000000) != 0): res = res * 0x2f16ac6c59de6f8d5d6f63c1482a7c8a1 // 0x1152aaa3bf81cb9fdb76eae12d0295732; # multiply by e^2^(+0)
    if ((_x & 0x40000000000000000000000000000000) != 0): res = res * 0x1152aaa3bf81cb9fdb76eae12d029572c // 0x02582ab704279e8efd15e0265855c47ab; # multiply by e^2^(+1)
    if ((_x & 0x80000000000000000000000000000000) != 0): res = res * 0x02582ab704279e8efd15e0265855c4792 // 0x000afe10820813d65dfe6a33c07f738f5; # multiply by e^2^(+2)
